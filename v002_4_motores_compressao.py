    ...
    def run(self):
        ...
        for idx in range(self.folder_listbox.count()):
            ...
            for output_path in output_paths:
                ...
                command = f'"{self.sevenzip_executable}" a -r -t7z -myx=9 -mx={self.compression_method} "{compressed_file_7z}" "{folder_path}"'

                if self.update_existing:
                    command = f'"{self.sevenzip_executable}" u -r -t7z -myx=9 -mx={self.compression_method} "{compressed_file_7z}" "{folder_path}"'
...
class Extracao(QThread):
    ...
    def __init__(self, winrar_executable, sevenzip_executable,
